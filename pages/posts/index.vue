        ...
        <div v-for="x in data.posts" :key="x.id" class="flex flex-col w-full max-w-md bg-slate-50 p-6 border-l-2 border-cyan-300">
          ...
          <div class="my-4 flex justify-between space-x-3 text-sm border-t-2 pt-2 px-2">
            <div class="border-l-2 pl-2 border-indigo-500">
              <span v-for="tag in x.tags" :key="tag" class="inline-flex pr-2 cursor-pointer hover:text-sky-500 decoration-slate-500">{{ tag }}</span>
            </div>
            <div class="hover:text-indigo-500 cursor-pointer">Like: {{ x.reactions }}</div>
          </div>
          <div class="">
...
<script setup>
const { pending, data } = await useFetch('https://dummyjson.com/posts')

// tag counts
const tagCounts = computed(() => {
  const tags = data.value.posts.reduce((acc, post) => {
